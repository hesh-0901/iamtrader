import { registerUser, loginUser, firebaseStatus } from './firebase-client.js?v=20260918-4';
...
function authScreen(mode='register',plan='free'){
const isLogin=mode==='login';
const p=plans[plan]||plans.free;
const title=isLogin?'Bon retour.':'Commencez simplement.';
...
form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),first=String(f.get('firstName')||'').trim(),pw=String(f.get('password')||''),msg=document.querySelector('#authMessage');if(first.length<2){msg.textContent='Veuillez renseigner un prénom valide.';msg.className='lp-auth-message show error';return}if(!isLogin){const cp=String(f.get('confirmPassword')||'');if(pw!==cp){msg.textContent='Les mots de passe ne correspondent pas.';msg.className='lp-auth-message show error';return}}if(!firebaseStatus().configured){msg.textContent='Firebase n’est pas encore configuré. Ajoutez la configuration Firebase dans firebase-config.js.';msg.className='lp-auth-message show error';return}msg.textContent='Connexion à Firebase…';msg.className='lp-auth-message show';const submit=form.querySelector('button[type="submit"]');if(submit)submit.disabled=true;try{const operation=isLogin?loginUser({firstName:first,password:pw}):registerUser({firstName:first,password:pw,plan,memberCode:String(f.get('memberCode')||'').trim()});const result=await Promise.race([operation,new Promise((_,reject)=>setTimeout(()=>reject(Object.assign(new Error('Firebase ne répond pas. Vérifiez la connexion ou le blocage de Firestore/Auth par une extension du navigateur.'),{code:'iamtrader/timeout'})),AUTH_TIMEOUT_MS))]);localStorage.setItem('iamtrader:session',JSON.stringify({uid:result.user.uid,firstName:result.profile.firstName,role:result.profile.role||'retail',plan:result.profile.plan||'free',requestedPlan:result.profile.requestedPlan||null,communityStatus:result.profile.communityStatus||null}));msg.textContent=isLogin?'Connexion réussie. Chargement de votre espace…':'Compte créé. Chargement de votre espace…';msg.className='lp-auth-message show success';navigate('home');}catch(error){const map={'auth/email-already-in-use':'Ce prénom est déjà utilisé. Choisissez un autre identifiant.','auth/invalid-email':'Prénom invalide.','auth/weak-password':'Le mot de passe est trop faible.','auth/invalid-credential':'Prénom ou mot de passe incorrect.','auth/user-disabled':'Ce compte a été désactivé.','permission-denied':'Accès Firestore refusé. Vérifiez les règles Firestore.','auth/operation-not-allowed':'La création de compte par mot de passe n’est pas activée dans Firebase Authentication.','auth/network-request-failed':'Firebase est inaccessible. Vérifiez votre connexion ou un bloqueur de contenu.','iamtrader/timeout':'Firebase ne répond pas après 15 secondes.'};msg.textContent=map[error?.code]||error?.message||'Une erreur Firebase est survenue.';msg.className='lp-auth-message show error';if(submit)submit.disabled=false}};
...
if(hash==='#app'||hash==='#home'||hash==='#home-settings'||hash==='#admin')return;
