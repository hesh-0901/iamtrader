...
(()=>{
  ...
  function render(force=false){
    ...
    const cells=[];
    for(let i=0;i<42;i++){
      const d=new Date(start);d.setDate(start.getDate()+i);const k=key(d),x=map.get(k)||{pnl:0,trades:0},inMonth=d.getMonth()===first.getMonth(),inRange=selected&&d>=selected.from&&d<=selected.to,today=k===key(new Date());
      cells.push(`<button type="button" class="cal3-day ${inMonth?'':'muted'} ${today?'today':''} ${inRange?'selected':''}" data-date="${k}"><b>${d.getDate()}</b><span class="cal3-spacer"></span><span class="cal3-pnl ${x.pnl>0?'win':x.pnl<0?'loss':'flat'}">${x.trades?`${x.pnl>=0?'+':''}${money(x.pnl,currency)}`:'—'}</span><small>${x.trades?`Trades: ${x.trades}`:'Aucun trade'}</small></button>`)
    }
