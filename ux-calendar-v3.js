/* IAMTRADER CALENDAR V3 — integrated period selection + range summary */
(()=>{
  ...
  function bind(){
    ...
    root.querySelector('[data-apply]').onclick=()=>{const f=root.querySelector('[data-from]').value,t=root.querySelector('[data-to]').value;if(!f||!t)return;if(f>t)return;const from=new Date(`${f}T00:00:00`),to=new Date(`${t}T00:00:00`);state.range={from,to};state.preset='custom';state.month=new Date(to.getFullYear(),to.getMonth(),1);render()};
